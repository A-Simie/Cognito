...
export const AjibadeAvatar: React.FC<AjibadeAvatarProps> = ({ isSpeaking }) => {
  return (
    <div className="flex flex-col items-center gap-3">
      <div className="relative">
        ...
        <div
          className={`absolute -inset-3 rounded-full blur-xl transition-all duration-500 ${
            isSpeaking
              ? "bg-linear-to-r from-cyan-400/50 to-blue-500/50 animate-pulse"
              : "bg-linear-to-r from-blue-500/30 to-blue-600/30"
          }`}
        />

        {/* Avatar frame */}
        <div
          className={`relative w-28 h-28 rounded-full overflow-hidden border-4 transition-all duration-300 ${
            isSpeaking
      ...
      {/* Status badge */}
      <div
        className={`flex items-center gap-2 px-4 py-1.5 rounded-full backdrop-blur-sm border transition-all duration-300 ${
          isSpeaking
